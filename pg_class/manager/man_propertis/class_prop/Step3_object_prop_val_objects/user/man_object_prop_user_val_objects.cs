...
namespace pg_class
{
    ...
    {
        #region МЕТОДЫ ОБЪЕКТОВ ПОЗИЦИИ И КЛАССА: ГРУППОВОЕ УПРАВЛЕНИЕ ЗНАЧЕНИЯМИ ПОЛЬЗОВАТЕЛЬСКИХ СВОЙСТВ КЛАССОВ ШАГ №03
        #region ДОБАВИТЬ

        /// <summary>
        /// Добавить новое значение пользовательского свойства объектам снимка класса в указанной позиции
        /// </summary>
        public position objects_prop_user_val_objects_add(position Position_parent, class_prop_user_val newClassPropUserVal, Boolean on_internal = false)
        {
            Int32 error;
            String desc_error;
            ...
            if (newClassPropUserVal != null)
            {
                switch (newClassPropUserVal.DataSize)
                {
                    case eDataSize.BigData:
                        cmdk = CommandByKey("object_prop_user_big_val_objects_add");
                    ...
                    case eDataSize.SmallData:
                        cmdk = CommandByKey("object_prop_user_small_val_objects_add");
        ...
        /// </summary>
        public Boolean object_prop_user_val_objects_add(out eAccess Access)
        {
