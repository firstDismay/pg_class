...
namespace pg_class
{
    ...
    {
        /// <summary>
        /// Лист возвращает список расширенных объектов по массиву идентификаторов объектов
        /// </summary>
        public List<object_general> object_ext_by_id_array(long[] object_array)
        {
            List<object_general> object_list = new List<object_general>();
            DataTable tbl_object = TableByName("vobject_general");
            NpgsqlCommandKey cmdk;

            cmdk = CommandByKey("object_ext_by_id_array");
            if (cmdk != null)
            {
                if (!cmdk.Access)
                {
        ...
        /// </summary>
        public Boolean object_ext_by_id_array(out eAccess Access)
        {
