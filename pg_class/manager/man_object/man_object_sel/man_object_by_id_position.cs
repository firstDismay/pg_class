...
namespace pg_class
{
    public partial class manager
    {
        ...
        /// Лист представлений объектов по идентификатору позиции
        /// </summary>
        public List<object_general> object_by_id_position(position Position, Boolean Extended = false)
        {
            List<object_general> Result;
            if (Extended)
            {
                Result = object_ext_by_id_position(Position);
            }
            else
            {
                Result = object_by_id_position(Position);
            }
            return Result;
        }
