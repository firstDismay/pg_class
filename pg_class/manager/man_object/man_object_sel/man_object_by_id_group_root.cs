...
namespace pg_class
{
    public partial class manager
    {
        ...
        /// Лист представлений объектов по идентификатору группы
        /// </summary>
        public List<object_general> object_by_id_group_root(group Group, Boolean Extended = false)
        {
            List<object_general> Result;
            if (Extended)
            {
                Result = object_ext_by_id_group_root(Group);
            }
            else
            {
                Result = object_by_id_group_root(Group);
            }
            return Result;
        }
