...
namespace pg_class
{
    public partial class manager
    {
        ...
        /// Лист объектов снимка класса по идентификатору класса и содержащей позиции
        /// </summary>
        public List<object_general> object_by_id_class_snapshot_id_pos(vclass Class, position Position_parent, Boolean Extended = false)
        {
            List<object_general> Result;
            if (Extended)
            {
                Result = object_ext_by_id_class_snapshot_id_pos(Class, Position_parent);
            }
            else
            {
                Result = object_by_id_class_snapshot_id_pos(Class, Position_parent);
            }
            return Result;
        }
