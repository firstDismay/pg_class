...
namespace pg_class
{
    public partial class manager
    {
        ...
        /// Лист представлений объектов ссылающихся на указанный объект, разрешение обратных ссылок
        /// </summary>
        public List<object_general> object_by_link_object(object_general Object_link, Boolean on_recursive, Boolean Extended = false)
        {
            List<object_general> Result;
            if (Extended)
            {
                Result = object_ext_by_link_object(Object_link, on_recursive);
            }
            else
            {
                Result = object_by_link_object(Object_link, on_recursive);
            }
            return Result;
        }
